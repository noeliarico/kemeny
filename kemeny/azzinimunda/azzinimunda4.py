class AzziniMunda4: 
    ...
    def __init__(self, om):
        ...
        self.ntentative = 0
        # boolean outranking matrix
        self.boolean_om = self.om > (self.m//2)
        # half of the voters
        self.half = (self.np.arange(self.n)*(self.m//2))[::-1]
        # condorcet
        self.cond = self.boolean_om.sum(axis = 1)
        print(self.cond)
    ...
    def AM4(self, level):
        ...
        for i in range(self.to_explore.size):
            if keep[i]:
                ...
                # update the condorcet ranking
                self.cond = self.cond - self.boolean_om[:,i]
                self.AM4(level+1) # recursive call
                # restore the condorcet ranking
                self.cond = self.cond + self.boolean_om[:,i]
    ...
    def check_if_recursive_condorcet(self):
        ...
        # return cr
        # opc2
        # cr = (self.boolean_om[:,self.to_explore].sum(axis=1))[self.to_explore]
        # thereis = self.np.all(self.np.in1d(self.np.arange(self.n), cr))
        # return None if not thereis else self.n-1-cr
        # opc3
        thereis = self.np.all(self.np.in1d(self.np.arange(self.to_explore.sum()), self.cond[self.to_explore]))
        
        return None if not thereis else self.to_explore.sum()-1-self.cond[self.to_explore]
