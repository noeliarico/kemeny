...
#' d_abstract:
#' -----------------------------------------------------------------------------
#' d1: Average Kendall distance between the rankings in the profile
#' -----------------------------------------------------------------------------
#' Indexes based on the range of positions
#' d2: Maximum range of positions of the alternatives
#' d3: Minimum range of positions of the alternatives
#' d4: Average range of positions of the alternatives
#' -----------------------------------------------------------------------------
#' d5: Bounds to the optimal cost
#' ----------------------------------------------------------------------------- 
#' d6: Intransitivity
#' ----------------------------------------------------------------------------- 
#' Indexes based on the pairwise opinion of the voters
#' d7: Mode margin
#' d8: Number of different margins
#' d9: Frequency of the minimum margin
#' ----------------------------------------------------------------------------- 
#' Indexes based on the rowsum of the outranking matrix
#' d10: Distance from the borda ranking to the profile
#' d11: Number of alternatives having rowsum >= colsum
#' d12: Sd of the rowsum
#' ----------------------------------------------------------------------------- 
#' Indexes based on the distribution of the Condorcet ranking (beta)
#' d13: sd de beta
#' d14: average
#' d15: median
#' d16: diferencia de mean-median


#' Abstract skeleton for all the measures of difficulty defined for CAEPIA 2021
#'
...
# Function to obtain the triangular number
...
# Equivalent to the lower bound and upper bound as they are equivalent if the
# sum of the matrix is constant
...
d5 <- function(profileOfRankings = NULL, om = NULL, absolute = TRUE) {
  ...
  if(absolute) {
    ...
    return((sum(mins))/(m*tr(n)))
  }
}


# Intransitive cycles
d6 <- function(profileOfRankings = NULL, om = NULL) {
  d <- d_abstract(profileOfRankings, om)
  om <- d$om
  half <- d$h
  #print(half)
  ...
  # cat(paste0("\nNumber of cycles checked: ", observed/6, "\n"))
  return(total/3)
}
...
d9 <- function(profileOfRankings = NULL, om = NULL, absolute = TRUE, trace = TRUE) {
  d <- d_abstract(profileOfRankings, om)
  om <- d$om
  m <- om[1,2]+om[2,1]
  half <- d$h
...
d10 <- function(profileOfRankings = NULL, om = NULL) {
  d <- d_abstract(profileOfRankings, om)
  om <- d$om
  half <- d$h
...
d11 <- function(profileOfRankings = NULL, om = NULL) {
  d <- d_abstract(profileOfRankings, om)
  om <- d$om
...
d16 <- function(profileOfRankings = NULL, om = NULL) {
  ...
  v <- rowSums(v)
  # mean of the rowsums
  return(abs(mean(v)-median(v)))
}


# posible nuevo indice?
